...
use Forumkit\Install\Installer;
use Forumkit\Install\InstallServiceProvider;
use Forumkit\Locale\LocaleServiceProvider;
use Forumkit\Settings\SettingsRepositoryInterface;
use Forumkit\Settings\UninstalledSettingsRepository;
...
class UninstalledSite implements SiteInterface
{
    ...
    protected function bootLaravel(): Container
    {
        ...
        $container->instance('forumkit.config', new Config(['url' => $this->baseUrl]));
        $container->alias('forumkit.config', Config::class);
        $container->instance('forumkit.debug', true);
    ...
    {
        $logPath = $this->paths->storage.'/logs/forumkit-installer.log';
        $handler = new StreamHandler($logPath, Logger::DEBUG);
        $handler->setFormatter(new LineFormatter(null, null, true, true));

        $container->instance('log', new Logger('Forumkit Installer', [$handler]));

...
use Forumkit\Admin\AdminServiceProvider;
use Forumkit\Api\ApiServiceProvider;
use Forumkit\Bus\BusServiceProvider;
use Forumkit\Console\ConsoleServiceProvider;
use Forumkit\Database\DatabaseServiceProvider;
use Forumkit\Discussion\DiscussionServiceProvider;
use Forumkit\Extension\ExtensionServiceProvider;
use Forumkit\Filesystem\FilesystemServiceProvider;
use Forumkit\Filter\FilterServiceProvider;
use Forumkit\Formatter\FormatterServiceProvider;
use Forumkit\Site\SiteServiceProvider;
use Forumkit\Frontend\FrontendServiceProvider;
use Forumkit\Group\GroupServiceProvider;
use Forumkit\Http\HttpServiceProvider;
use Forumkit\Locale\LocaleServiceProvider;
use Forumkit\Mail\MailServiceProvider;
use Forumkit\Notification\NotificationServiceProvider;
use Forumkit\Post\PostServiceProvider;
use Forumkit\Queue\QueueServiceProvider;
use Forumkit\Search\SearchServiceProvider;
use Forumkit\Settings\SettingsServiceProvider;
use Forumkit\Update\UpdateServiceProvider;
use Forumkit\User\SessionServiceProvider;
use Forumkit\User\UserServiceProvider;
...
class InstalledSite implements SiteInterface
{
    ...
    /**
     * @var \Forumkit\Extend\ExtenderInterface[]
     */
    protected $extenders = [];
    ...
    /**
     * @param \Forumkit\Extend\ExtenderInterface[] $extenders
    ...
    protected function bootLaravel(): Container
    {
        ...
        $container->instance('forumkit.config', $this->config);
        $container->alias('forumkit.config', Config::class);
        $container->instance('forumkit.debug', $this->config->inDebugMode());
        $container->instance('config', $config = $this->getIlluminateConfig());
        $container->instance('forumkit.maintenance.handler', new MaintenanceModeHandler);
        ...
        $laravel->booting(function () use ($container) {
            // 在启动服务提供商之前运行所有本地站点的扩展器
            // (但在 "real" 扩展的扩展器之后，这些扩展器已经在上面的服务提供商中设置好了)

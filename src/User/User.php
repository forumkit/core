...
class User extends AbstractModel
{
    ...
    /**
     * 一个可调用对象的数组，每个对象都会处理用户的组列表后再返回
     */
    protected static $groupProcessors = [];

    /**
     * 注册的用户偏好的数组。每个偏好由键定义，其值是一个数组，包含以下键：
     *
     * - transformer: 一个回调函数，用于限制偏好的值
     * - default: 如果偏好未设置时的默认值
     *
     * @var array
     */
    ...
    /**
     * 在调用`updateLastSeen()`方法前，与当前`last_seen`属性值之间的差异（以秒为单位），
     * 只有当差异大于此值时，才会在数据库中更新该属性
     */
    ...
    public static function boot()
    {
        ...
        static::deleting(function (self $user) {
            if ($user->id == 1) {
                throw new DomainException('不能删除根管理员');
            }
        });

        // 当用户被删除后，触发相应的事件和删除相关通知
    ...
    /**
     * 获取此用户所有已注册首选项的值，方法是转换其存储的首选项并将其与默认值合并。
     *
     * @param string|null $value
     * @return array
     */
     ...
     * 确保当前用户被允许执行某项操作。
     *
     * 如果条件不满足，将抛出一个异常，表示权限不足。这是关于授权的，即如果没有权限变更（或使用另一个用户帐户），重试此类请求/操作是毫无意义的。
     *
     ...
     * 确保给定的用户已通过身份验证。
     *
     * 这将为游客用户抛出一个异常，表示授权失败。因此，他们可以在登录后（或使用其他身份验证方式）重试该操作。
     *
    ...
    public function permissions()
    {
        ...
        // 如果用户的账户尚未激活，他们本质上只是访客。如果已激活，我们可以给他们标准的'member'组，以及他们被分配到的任何其他组。
        if ($this->is_email_confirmed) {
    ...
    /**
     * 获取一个偏好的键，该偏好标记用户是否将通过 $method 接收 $type 的通知。
     *
     * @param string $type
     * @param string $method
     * @return string

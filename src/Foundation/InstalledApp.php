...
class InstalledApp implements AppInterface
{
    ...
    public function getRequestHandler()
    {
        if ($this->config->inMaintenanceMode()) {
            return $this->container->make('forumkit.maintenance.handler');
        ...
        $pipe->pipe(
            new BasePathRouter([
                $this->subPath('api') => 'forumkit.api.handler',
                $this->subPath('admin') => 'forumkit.admin.handler',
                '/' => 'forumkit.forum.handler',
            ])
        );
    ...
    protected function getUpdaterHandler()
    {
        ...
        $pipe->pipe(new BasePath($this->basePath()));
        $pipe->pipe(
            new HttpMiddleware\ResolveRoute($this->container->make('forumkit.update.routes'))
        );
    ...
    public function getConsoleCommands()
    {
        return array_map(function ($command) {
            ...
            return $command;
        }, $this->container->make('forumkit.console.commands'));
    }
}

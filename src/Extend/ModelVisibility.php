...
class ModelVisibility implements ExtenderInterface
{
     ...
     *                           This model must extend from \Forumkit\Database\AbstractModel,
     *                           and use \Forumkit\Database\ScopeVisibilityTrait.
     */
    public function __construct(string $modelClass)
    {
        ...
        if (class_exists($this->modelClass) && ! is_callable([$modelClass, 'registerVisibilityScoper'])) {
            throw new Exception("Model $modelClass cannot be visibility scoped as it does not use Forumkit\Database\ScopeVisibilityTrait.");
        }
    }

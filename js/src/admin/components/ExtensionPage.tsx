...
export default class ExtensionPage<Attrs extends ExtensionPageAttrs = ExtensionPageAttrs> extends AdminPage<Attrs> {
        ...
        <div className="container">
          <div className="ExtensionTitle">
            ...
            <div className="ExtensionName">
              <h2>{this.extension.extra['forumkit-extension'].title}</h2>
            </div>
  ...
  onerror(e: RequestError) {
    // 我们需要给模态框动画开始的时间；如果我们过早地关闭模态框，
    // 这会破坏 bootstrap 模态框库。
    // TODO: 当我们不再使用 bootstrap JS 来管理模态框时，这个临时解决方案应该被移除。
    setTimeout(() => {
      app.modal.close();
    }, 300); // Bootstrap 的 Modal.TRANSITION_DURATION 是 300 毫秒。

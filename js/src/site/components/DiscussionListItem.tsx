...
export default class DiscussionListItem<CustomAttrs extends IDiscussionListItemAttrs = IDiscussionListItemAttrs> extends Component<CustomAttrs> {
  ...
  authorAvatarView(): Mithril.Children {
    ...
    return (
      <Tooltip
        text={app.translator.trans('core.site.discussion_list.posted_on_text', { user, ago: humanTime(discussion.createdAt()) })}
        position="right"
      >
  ...
  /**
   * 确定是否应显示关于谁发起了讨论的信息，而不是显示关于讨论的最新回复的信息。
   */
  showFirstPost() {
  ...
  /**
   * 构建讨论列表的信息项列表。默认情况下，这仅包含第一个/最后一个帖子的指示器。
   */
